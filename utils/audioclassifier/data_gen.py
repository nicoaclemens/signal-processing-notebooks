...
def build_dataset_streaming(
    ...
    y_label = np.memmap(
        out_path + "_y_label.dat", dtype=np.int64, mode="w+", shape=(total_samples,)
    )
    y_type = np.memmap(
        out_path + "_y_type.dat", dtype=np.int64, mode="w+", shape=(total_samples,)
    )
...
if __name__ == "__main__":
    ...
    base_dir = "./out/audioclassifier/datagen/"

    N_TOTAL_PER_CLASS = 10_000

    splits = {
        base_dir + "trainingdata-large": int(0.8 * N_TOTAL_PER_CLASS),
        base_dir + "validationdata-large": int(0.1 * N_TOTAL_PER_CLASS),
        base_dir + "testingdata-large": int(0.1 * N_TOTAL_PER_CLASS),
    }
